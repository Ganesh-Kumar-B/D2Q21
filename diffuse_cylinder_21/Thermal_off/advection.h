...
template<typename T>
void Mark_for_cylinder(Grid_N_C_2D<T> &marker, int ic, int jc, double L){

std::ofstream Myfiles("solid_points.txt");
    L = L/2;
double r2 = 0;
    ...
    for(int i = 0 + marker.noghost; i < marker.n_x_node - (marker.noghost) ; i++){
        for(int j = 0 + marker.noghost;j < marker.n_y_node - (marker.noghost) ; j++){
            ...
            r2 = (i+0.5 - ic)*(i+0.5 - ic) + (j+0.5 - jc)*(j+0.5 - jc);
...
template<typename T>
void periodic_x(Grid_N_C_2D<T> &grid, lbmD2Q21<T> &lb){


for(int j = 0; j < grid.n_y_node; j++)
    for(int i = 0; i<grid.noghost; i++){


        for(int dv = 0; dv< grid.d_v; dv++){


            grid.Node(i,j,dv) = grid.Node(i + grid.n_x, j , dv);
            grid.Cell(i,j,dv) = grid.Cell(i + grid.n_x, j , dv);

            grid.Node( (grid.n_x_node-1) - i ,j , dv) = grid.Node((grid.n_x_node-1) - i - grid.n_x ,j ,dv );
            grid.Cell( (grid.n_x_node-1) - i ,j , dv) = grid.Cell((grid.n_x_node-1) - i - grid.n_x ,j ,dv );
...
template<typename T>
void periodic_y(Grid_N_C_2D<T> &grid, lbmD2Q21<T> &lb){


for(int i = 0; i < grid.n_x_node; i++)
    for(int j = 0; j<grid.noghost; j++){


        for(int dv = 0; dv< grid.d_v; dv++){


            grid.Node(i,j,dv) = grid.Node(i,j + grid.n_y, dv);
            grid.Cell(i,j,dv) = grid.Cell(i,j + grid.n_y, dv);

            grid.Node(i, (grid.n_y_node-1) - j ,dv) = grid.Node(i, (grid.n_y_node-1) - j - grid.n_y , dv );
            grid.Cell(i, (grid.n_y_node-1) - j ,dv) = grid.Cell(i, (grid.n_y_node-1) - j - grid.n_y , dv );
...
double feq[21] = {0};
get_equi(feq, lb,0.0,0.0,2.0,lb.theta0);
for(int i = left ; i < right;i++)
    for(int j = bottom ; j < top; j++){
...
template<typename T, typename T1>
void u_free_stream(Grid_N_C_2D<T>  &grid,Grid_N_C_2D<T1>  &marker, lbmD2Q21<T> &lb, double &u_free, double &rho_avg){
    ...
    for(int  i = grid.noghost; i< grid.n_x_node -grid.noghost; i++)
        for(int j = grid.noghost; j< grid.n_y_cell - grid.noghost; j++){

            

            if(marker.Node(i,j,0) == FLUID)
                for(int dv = 0; dv<grid.d_v; dv++)
                    rho_avg += grid.Node(i,j,dv);
