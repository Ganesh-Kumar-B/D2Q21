...
{ 
  ...
  std::ofstream file;
  char fileName[250];
  sprintf(fileName,"./sod/velocity_%d.txt",step) ;
  file.open(fileName) ;
  file<<"x,y,ux,uy,rho,theta,p"<<std::endl;
  for(int i = 0 + gridLB.noghost; i < gridLB.n_x_node - (gridLB.noghost); i++) 
	for (int j = 0 + gridLB.noghost; j < gridLB.n_y_node - (gridLB.noghost); j++)
		{{
    ...
    // if(fabs(u4) < 0.000000000001){u4 = 0;}
      ...
      file<<(double)(i-gridLB.noghost)*ny_inv  <<"," <<(double)(j-gridLB.noghost) *ny_inv   <<","<<u1*u_inv<<","<<u2*u_inv<<","<<rho1<<","<<theta1<<","<<rho1*theta1<<std::endl;
      file<<(double)(i+0.5-gridLB.noghost)*ny_inv <<","<<(double)(j + 0.5-gridLB.noghost) *ny_inv <<","<<u3* u_inv<<","<<u4 * u_inv<<","<<rho2<<","<<theta2<<","<<rho2*theta2<<std::endl;
...
int main()
{
           /////////////try to use typedef next time
    int Nx =5000;int Ny =10;
    ...
    //------------------------------Main code--------------------------//
    initialization_shock(grid,d2q21,0.0);
    printMass(grid);
    printdata(d2q21,grid,0,1.0);
...
int sim_time = 20*Nx/u0; double time;
      ...
      for(int t = 1; t <= 2000;t++){
        ...
        inlet_shock(grid,d2q21,0.0,0.0,1.0);
        outlet_shock(grid,d2q21,0.0,0.0,1.0);
